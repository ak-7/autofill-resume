class AutoFill {
                ...
                if (field.type == "select-one") {
                    for (var i = 0; i < field.options.length; i++) {
                        if (Array.isArray(this.resume_json[key])) {
                            ...
                            console.log("L: 46", label, key, field.options[i])
                            if (field.options[i].value != "") {
                                if (field.options[i].text.toLowerCase().includes(this.resume_json[key].toLowerCase())) {
                                    console.log("L: 46", field.options[i], key)
                                    field.options[i].selected = true;
                                    field.options[i].dispatchEvent(new Event("change", { bubbles: !0, cancelable: !1 }));
                                    return;
                                }
                                else {
                                    if (this.resume_json[key].toLowerCase().includes(field.options[i].text.toLowerCase())) {
                                        console.log("L: 46", field.options[i], key)
                                        field.options[i].selected = true;
                                        field.options[i].dispatchEvent(new Event("change", { bubbles: !0, cancelable: !1 }));
                                        return;
                    ...
                    if (Array.isArray(this.resume_json[key])) {
                        ...
                        console.log("L: 80", field.parentNode.innerText, "check for", this.resume_json[key])
                        if (field.parentNode.innerText.toLowerCase().trim().includes(this.resume_json[key].toLowerCase().trim())) {
                            field.checked = true;
                            field.dispatchEvent(new Event("change", { bubbles: !0, cancelable: !1 }));
                            return;
                        }
    ...
    fill_demographic() {
        this.form = document.getElementById("demographic_questions");
        // var labels = this.form.getElementById("demographic_questions");
        // Get all label elements within the form
        var labels = this.form.querySelectorAll("div");

        // Check the number of labels found
        console.log("Total Labels found:", labels.length);

        // // Iterate over each label and log its content
        // labels.forEach(function (label) {
        //     console.log("Label text:", label.textContent.trim());
        // });
        console.log("L: 105, fill_demographic")

        for (var i = 0; i < labels.length; i++) {
            var label = labels[i].parentNode.innerText.split("\n")[0]
            var elements = labels[i].parentNode.getElementsByTagName("input");
            console.log("L: 109", label, elements)
            ...
            console.log("L: 123", label, elements)
            for (var j = 0; j < elements.length; j++) {
                var element = elements[j];
                if (element != null && element.type != 'hidden') {
                    this.fill_select_field(element, label);
        ...
        for (var i = 0; i < labels.length; i++) {
            ...
            for (var j = 0; j < elements.length; j++) {
                ...
                if (element != null && element.type != 'hidden') {
                    if (element.type == "text" || element.type == "email") {
                        ...
                        console.log("L: 142", label, elements)
                        this.fill_select_field(element, label);
                    }
                }
                else {
                    console.log("L: 157", label, elements)
                    this.fill_select_field(element, label);
                }
            }
            var elements = labels[i].parentNode.getElementsByTagName("select");
            for (var j = 0; j < elements.length; j++) {
                ...
                if (element != null && element.type != 'hidden') {
                    ...
                    console.log("L: 155", label, elements)
                    this.fill_select_field(element, label);
                }
            }
        }
